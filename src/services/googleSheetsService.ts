    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      try {
        const checkin: CheckinData = {
          MemberID: String(row[getColumnIndex('MemberID')] || '').trim(),
          FirstName: String(row[getColumnIndex('FirstName')] || '').trim(),
          LastName: String(row[getColumnIndex('LastName')] || '').trim(),
          Email: String(row[getColumnIndex('Email')] || '').trim(),
          OrderAt: String(row[getColumnIndex('OrderAt')] || '').trim(),
          Paid: parseFloat(String(row[getColumnIndex('Paid')] || '0')),
          PaymentMethodName: String(row[getColumnIndex('PaymentMethodName')] || '').trim(),
          CheckedIn: String(row[getColumnIndex('CheckedIn')] || '').toUpperCase() === 'TRUE',
          Complementary: String(row[getColumnIndex('Complementary')] || '').toUpperCase() === 'TRUE',
          IsLateCancelled: String(row[getColumnIndex('IsLateCancelled')] || '').toUpperCase() === 'TRUE',
          SessionID: String(row[getColumnIndex('SessionID')] || '').trim(),
          SessionName: String(row[getColumnIndex('SessionName')] || '').trim(),
          Capacity: parseInt(String(row[getColumnIndex('Capacity')] || '0')) || 0,
          Location: String(row[getColumnIndex('Location')] || '').trim(),
          Date: String(row[getColumnIndex('DateIST')] || row[getColumnIndex('Date')] || '').trim(),
          DayOfWeek: String(row[getColumnIndex('DayofWeek')] || row[getColumnIndex('DayOfWeek')] || '').trim(),
          Time: String(row[getColumnIndex('Time')] || '').trim(),
          DurationMinutes: parseInt(String(row[getColumnIndex('DurationMinutes')] || '0')) || 0,
          TeacherName: String(row[getColumnIndex('TeacherName')] || '').trim(),
          CleanedProduct: String(row[getColumnIndex('CleanedProduct')] || '').trim(),
          CleanedCategory: String(row[getColumnIndex('CleanedCategory')] || '').trim(),
          CleanedClass: String(row[getColumnIndex('CleanedClass')] || '').trim(),
          HostID: parseInt(String(row[getColumnIndex('HostID')] || '0')) || 0,
          Month: String(row[getColumnIndex('Month')] || '').trim(),
          Year: parseInt(String(row[getColumnIndex('Year')] || '0')) || 0,
          ClassNo: parseInt(String(row[getColumnIndex('ClassNo')] || '0')) || 0,
          IsNew: String(row[getColumnIndex('IsNew')] || '').trim(),
          UniqueID1: String(row[getColumnIndex('UniqueID_1')] || row[getColumnIndex('UniqueID1')] || '').trim(),
          UniqueID2: String(row[getColumnIndex('UniqueID2')] || '').trim(),
        };
